...
use dbus::MessageItem::{Str, Variant};
use dbus::{Connection, Message, MessageItem, Path};
use hkdf::Hkdf;
    ...
    pub fn new(bus: Rc<Connection>, encryption: EncryptionType) -> ::Result<Self> {
        match encryption {
            EncryptionType::Plain => {
                ...
                // Get session output
                let session_output_dbus = try!(items.get(0).ok_or(SsError::NoResult));
                ...
                // get session path
                let object_path_dbus = try!(items.get(1).ok_or(SsError::NoResult));
