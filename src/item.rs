...
impl<'a> Item<'a> {
    ...
    pub fn get_secret(&self) -> ::Result<Vec<u8>> {
        ...
        let secret_struct = try!(res.get(0).ok_or(SsError::NoResult));
        ...
        let secret_dbus = try!(secret_vec.get(2).ok_or(SsError::NoResult));
        ...
        if !self.session.is_encrypted() {
            ...
            let aes_iv_dbus = try!(secret_vec.get(1).ok_or(SsError::NoResult));
            // get array of dbus bytes
    ...
    pub fn get_secret_content_type(&self) -> ::Result<String> {
        ...
        let secret_vec: &Vec<_> = secret_struct.inner().unwrap();
        let content_type_dbus = try!(secret_vec.get(3).ok_or(SsError::NoResult));
